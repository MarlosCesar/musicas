...
style.textContent = `
  ...
  .tab {
    flex-shrink: 0;
    overflow: visible;
    position: relative; /* Para posicionamento absoluto do popup */
...
function renderTabs() {
  ...
  popup.className = "tab-popup-actions";
      ...
      btn.onclick = (e) => {
        e.stopPropagation(); // Descomentado para evitar propagação
        if (tab?.type === "custom") {
          editingTabIndex = idx;

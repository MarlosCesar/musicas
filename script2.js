// ===========================
// Cifras2 - Gerenciador de Cifras
// script2.js - MarlosCesar
// ===========================
...
const POLL_INTERVAL = 5000; // 5 segundos para polling de abas online (pode ajustar)
...
let state = {
  tabs: [...TABS_DEFAULT],     // Abas disponíveis
  cifras: {},                  // Cifras por aba
  selection: {},               // Seleção por aba
  currentTab: "Domingo Manhã", // Aba ativa
  search: "",                  // Texto de busca
  onlineCache: {},             // Cache para abas online (futuro)
};
...
// Altera modo da aba (offline/online no futuro)
...
// Limpa seleção de cifras na aba
function clearSelection(tab) {
  state.selection[tab] = new Set();
  updateFloatControls();
}

// ---- UI: RENDERIZAÇÃO DAS ABAS E CIFRAS ----

// Renderiza abas no topo
function renderTabs() {
  ...
  state.tabs.forEach((tab, idx) => {
    ...
    if (tab.type === "default") {
      // Botão "mais" para editar modo da aba default
...
// Renderiza lista de cifras na aba ativa
...
// Atualiza controles flutuantes para seleção de cifras
...
// ---- UPLOAD DE ARQUIVOS VIA FILE INPUT ----
...
// Adiciona cifra da nuvem à aba atual
...
// ---- FULLSCREEN PARA CIFRA DE IMAGEM COM OCR/TRANSPOSE ----
function openFullscreen(cifra) {
  ...
  function renderOverlays() {
    ...
    notesData.forEach(note => {
      ...
      div.innerHTML = transposeChord(note.text, currentTone);
  ...
  function detectNotes() {
    ...
    overlayNotes.innerHTML = "";
    // Usa Tesseract para OCR na imagem (proxificada)
  ...
  // Controles de transposição de tom
...
function abrirCifraTextoFullscreen() {
  ...
  overlay.innerHTML = `
    ...
    <div style="position:relative;width:100vw;height:100vh;display:flex;align-items:center;justify-content:center;">
      <pre id="cifra-texto-full" style="font-size:1.1em;max-width:90vw;max-height:80vh;overflow:auto;background:#fff;color:#222;padding:25px 18px 18px 18px;border-radius:12px;box-shadow:0 2px 16px #0003;"></pre>
  ...
  // Triplo clique para mostrar controles de tom
...
// ---- POLLING PARA ABAS ONLINE (placeholder) ----
